...
export const dislikeComment = createAsyncThunk(
  ...
  async (commentId, thunkAPI) => {
    try {
      ...
      return await commentService.dislikeComment(commentId, token);
    } catch (error) {
      console.error(error);
      (error.response && error.response.data && error.response.data.message) ||
        error.message ||
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(likeComment.fulfilled, (state, action) => {
        state.isLoading = false;
        state.isSuccess = true;
        // Handle updating the like count for the specific comment in the state
      ...
      .addCase(dislikeComment.fulfilled, (state, action) => {
        state.isLoading = false;
        state.isSuccess = true;
        // Handle updating the dislike count for the specific comment in the state
        const commentId = action.payload.commentId;
        const comment = state.comments.find((c) => c._id === commentId);
        if (comment) {
